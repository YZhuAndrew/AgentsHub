<div align="center">
  <img src="../assets/readme/hero.png" width="100%" alt="AgentsHub — a local-first workspace for prompts, skills, and AI coding assets" />
...
Desktop builds are published on GitHub Releases for macOS / Windows / Linux.
...
| Linux | See [Releases](https://github.com/YZhuAndrew/AgentsHub/releases) |

> **macOS?** Only Apple Silicon (M1/M2/M3/M4) `arm64` builds are published.
...
> The screenshots below cover the main desktop workspaces: Prompt, Skill, Agent, MCP, Plugin, and Rules.
...
- Fixed skill distribution selections being reset immediately on the Skill detail page: platform choices are no longer cleared by background install-status refreshes, so global/project distribution completes normally
- Fixed skill preview scroll stutter: the auto safety scan no longer loops scan → save → rescan, and unrelated re-renders no longer re-parse and re-highlight the whole markdown preview
...
- Upgrade backup fix: the pre-upgrade backup no longer aborts on the first symbolic link (e.g. symlink-mode Skill installs); it preserves internal symlinks, skips escaping ones, and handles macOS /var path normalization
...
- Skill list view enhancement: the list view is now the default, with a column-header table layout (name + description / source / author / version / created / updated / platform status / actions), an author filter, and a "check all updates" batch action with batch update for selected skills
- Git Skill import progress: installing skills from a Git repository now reports detailed progress for both the Scan and Import phases (phase label, `index / total` + skill name, live clone percentage), replacing the single opaque spinner

### v0.6.1 (2026-08-10, stable)

- Added the QwenWork built-in agent platform target so Skills can be distributed to it directly
- Startup behavior settings: the main window opens by default on launch, a new "startup view" selector (defaults to last-used), and launch-at-system-startup

### v0.6.0 (2026-08-09, stable)

- First AgentsHub fork baseline; aligned the Desktop, CLI, self-hosted Web, Cloudflare Worker, and Mobile build versions
...
Thanks to everyone who has contributed to AgentsHub.
...
If AgentsHub is useful to your work, consider buying the maintainer a coffee ☕
...
AgentsHub is forked from [PromptHub](https://github.com/legeling/PromptHub) (AGPL-3.0). Thanks to the original author [legeling](https://github.com/legeling) for the open-source contribution. This project extends it with Agent asset management, CLI diagnostics, and usage monitoring.

---

<div align="center">
  <p>If AgentsHub is useful to you, a ⭐ goes a long way.</p>
</div>
