...
AgentsHub est un build fork maintenu par la communauté, sans signature Apple Developer. Téléchargez-le depuis GitHub Releases ; macOS Gatekeeper peut bloquer le premier lancement.

Si macOS indique que AgentsHub est endommagé ou que le développeur ne peut pas être vérifié, exécutez :
...
Vous voulez tester la prochaine version d'aperçu ? Ouvrez _Paramètres → À propos_ et activez le canal aperçu. L'application vérifiera alors les Prereleases GitHub. Désactivez-le pour revenir à la stable ; AgentsHub ne fait pas de retour automatique d'une preview plus récente vers une stable plus ancienne.
...
- AgentsHub Web auto-hébergé conserve séparément des snapshots immuables ; les tâches au lancement et planifiées envoient uniquement et ne récupèrent ni n'écrasent les données locales
...
3. **Installez vers vos outils IA.** Depuis le détail d'un Skill, sélectionnez la plateforme cible. AgentsHub installe le SKILL.md dans le dossier attendu par la plateforme, en symlink (édition partagée) ou en copie indépendante.

4. **Synchronisation ou sauvegarde (optionnelle).** _Paramètres → Données_ configure WebDAV / S3 pour la sync active, ou AgentsHub Web auto-hébergé pour des snapshots de récupération indépendants.

<div id="self-hosted-web"></div>

## Web auto-hébergé

AgentsHub Web est un compagnon léger orienté navigateur, à exécuter sur un NAS, un VPS ou une machine LAN avec Docker. Ce **n'est pas** un service cloud managé. Utilisez-le pour :

- Accéder à vos données AgentsHub depuis un navigateur
...
- `--data-dir <path>` — surcharger le dossier `userData` de AgentsHub
...
- Corrigé la régression de performances de la v0.8.0 : la réconciliation des espaces de travail canoniques ne reconstruit plus tous les espaces de travail de skills à chaque initialisation de la base de données — le démarrage redevient rapide et la page Skills ne se gèle plus
- Corrigé la régression de la v0.8.0 qui verrouillait le démarrage pour les skills importées contenant des liens symboliques relatifs intra-paquet (ex. l'alias `AGENTS.md -> CLAUDE.md`) ; les liens contenus sont matérialisés dans les projections ou recréés avec des cibles relatives dans les snapshots, les liens s'échappant restent refusés
...
- Icône permanente de la barre d'état : un nouveau commutateur optionnel garde l'icône de la barre des menus / tray visible même fenêtre ouverte ; compatible avec la minimisation et appliqué en direct

### v0.7.1 (2026-08-13, stable)

- Correction de la sauvegarde de mise à niveau : la sauvegarde pré-mise à niveau n'avorte plus au premier lien symbolique (ex. skill installé en mode symlink) ; les liens internes sont préservés, les liens sortants ignorés, et la normalisation /var de macOS est gérée

### v0.7.0 (2026-08-13, stable)

- Import de compétences par lot : un nouveau mode d'import par lot installe plusieurs archives ZIP locales (glisser-déposer ou sélecteur) et/ou plusieurs URLs GitHub/Git à la fois ; les ZIP locaux réutilisent le même cycle d'installation atomique que les paquets distants ; déposer des ZIP sur My Skills ouvre l'import par lot
- Visibilité des plateformes unifiée avec le bouton Settings (la détection devient un indice ; copilot/amp ajoutés à l'ordre) ; ajout de la plateforme QwenWork CN ; correction de la racine par défaut de trae-work-cn vers ~/.trae-cn

### v0.6.2 (2026-08-11, stable)

- Amélioration de la vue en liste des Skills : la vue en liste devient l'affichage par défaut, avec une mise en forme tabulaire à en-têtes de colonnes (nom + description / source / auteur / version / création / mise à jour / état plate-forme / actions), un filtre par auteur et une action groupée « tout vérifier » avec mise à jour par lot des Skills sélectionnés
- Progression de l'import Git : l'installation de Skills depuis un dépôt Git affiche une progression détaillée pour les phases d'analyse et d'import (libellé de phase, `index/total` + nom du Skill, pourcentage de clonage en temps réel), en remplacement du seul indicateur figé

### v0.6.1 (2026-08-10, stable)

- Ajout de la cible de plate-forme Agent intégrée QwenWork, permettant la distribution directe de Skills
- Réglages de comportement au démarrage : la fenêtre principale s'ouvre par défaut au lancement, un nouveau sélecteur de « vue de démarrage » (par défaut la dernière utilisée) et le lancement automatique au démarrage du système

### v0.6.0 (2026-08-09, stable)

- Première version de base du fork AgentsHub ; alignement des versions de build Desktop, CLI, Web auto-hébergé, Cloudflare Worker et Mobile
...
- Le flux macOS documente l'installation en fork non signé, la vérification DMG/ZIP et la gestion Gatekeeper
...
- Quand une installation symlink retombe en copy, AgentsHub affiche désormais un warning explicite au lieu de ressembler à un succès normal
...
- Arbres de relations Prompt, chemins Windows Agent, toggle captcha Web, l'installation macOS en fork non signé et correctifs de release pipeline arrivent aux utilisateurs stables
...
- [ ] Extension navigateur qui interroge AgentsHub depuis ChatGPT / Claude
