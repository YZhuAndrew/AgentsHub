...
import { resolveInitialBounds } from "./window-state";
import {
  attachWindowStateCapture,
  flushWindowStateSave,
} from "./window-state-capture";
...
async function createWindow() {
  ...
  // Seed the geometry tracker, restore the maximized state before the window
  // becomes visible (to avoid a resize flash), and attach the capture listeners.
  // 在窗口可见前恢复最大化状态、初始化几何追踪器并挂载监听。
  attachWindowStateCapture(mainWindow, () => appDb, {
    maximize: restoredBounds?.maximized ?? false,
  });
...
app.on("before-quit", () => {
  ...
  if (mainWindow && !mainWindow.isDestroyed()) {
    flushWindowStateSave(mainWindow, () => appDb);
  }
  closeDatabase();
});
