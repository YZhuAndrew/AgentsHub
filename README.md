...
- 紧急修复启动白屏：用 Finder 浏览本地数据目录后生成的 `.DS_Store` 不再让启动失败；打包版渲染进程的跨 chunk 初始化环已消除，安装包可正常打开
- 性能优化：Prompt 变更只同步受影响的工作区文件（不再全量重写）、复制/收藏不再隐性重写全文索引、启动完整性扫描减半、Markdown 热路径跳过无变化重解析、AI 测试流式输出节流、markdown 依赖移出首屏、自托管 Web 静态缓存与 gzip


### v0.8.2（2026-08-15，正式版）

- 修复技能详情页分发选择被立即重置：选择目标平台后不再被后台安装状态刷新清空，全局/项目分发可正常完成
- 修复技能预览滚动卡顿：自动安全扫描不再陷入「扫描→保存→重扫」循环，无关重渲染不再触发整篇 Markdown 重新解析与高亮
